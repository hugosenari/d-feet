...
dist = setup(name='d-feet',
    ...
    platforms='linux',
    scripts=['d-feet'],
    packages=['dfeet', 'dfeet/_ui', 'dfeet/_ui/dbustoany'],
    data_files=[
        ('/usr/share/dfeet', glob.glob("ui/*.ui")),
        ('/usr/share/dfeet', glob.glob("xslt/*.xsl")),
        ('/usr/share/icons/hicolor/16x16/apps', glob.glob("icons/16x16/*.png")),
        ('/usr/share/icons/hicolor/24x24/apps', glob.glob("icons/24x24/*.png")),
        ('/usr/share/icons/hicolor/32x32/apps', glob.glob("icons/32x32/*.png")),
        ('/usr/share/icons/hicolor/48x48/apps', glob.glob("icons/48x48/*.png")),
        ('/usr/share/icons/hicolor/256x256/apps',
            glob.glob("icons/256x256/*.png")),
        ('/usr/share/applications', ['ui/dfeet.desktop']),
...
if omfdir is not None and installdir is not None and dist is not None:
...
print("\nInstallation finished! You can now run d-feet by typing 'd-feet'")
print("or through your applications menu icon.")

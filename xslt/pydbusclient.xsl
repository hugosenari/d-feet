...
* bus_name: <xsl:value-of select="$bus_name"/>
* object_path: <xsl:value-of select="$object_path"/>
...
<xsl:if test="$interface = string(@name) or $interface = ''">
